.data:
	...
	zapytanie_o_instrukcje: .asciiz "\nPodaj instrukcje: (Mozliwe instrukcje to: ADD, ADDI, J, NOOP, MULT, JR, JAL) \n"
...
.text:
	...
	umieść_instrukcje_na_stosie:
		...
		umieść_linie_oddzielajaca:
			# Umieszczenie linii oddzielającej na stosie
			la $t0, linia_oddzielajaca
			addi $sp, $sp, -4
			sw $t0, ($sp)
			
			# Aktualizacja licznika pamięci
			add $s5, $s5, 1
			
			j pobierz_kolejna_instrukcje
	...
	obsluz_add:
		...
		# Instrukcja poprawna - umieść na stosie
		j umieść_instrukcje_na_stosie
